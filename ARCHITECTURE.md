...
README.md (player-facing). **Status and next work live in `HANDOFF.md`**,
not in the TODO comments below — some of those comments predate Phases 13–15.
...
- **Rotate v2 and reusable subassemblies shipped (Phase 13).** Remaining
  editor TODOs: part search/category tabs, pad-clearance warnings.
- Surface attach (Phase 13 follow-up): legs / battery / solar / clamps may
  mate flush to a host hull without a node pair (`vehicle/SurfaceAttach.ts`).
- Chrome (Phase 15, frozen): black / square / hairline tokens in `style.css`.
...
- **Launch clamps**: category `clamp`, treated as a release part in
  ...
  the vessel to the pad with **infinite hold** (KSP-cheaty; mass/TWR ignored)
  until the clamp stage fires. Tower height and umbilical length are
